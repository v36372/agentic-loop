...
Before doing any other Effect-related work, check that `./.agent_sources/effect` exists at the root of the repository where the skill is being used.
...
3. Effect source code last. For gaps in the guides, complex type errors, unclear behavior, or implementation details, examine the vendored Effect source at `./.agent_sources/effect/packages/effect/src/`.
...
- Use the listed repo paths to jump directly into the vendored source under `./.agent_sources/effect`.
...
These guides should be treated as the default implementation guidance. Do not skip them and jump straight to `./.agent_sources/effect` unless you need source-level confirmation or the guides do not answer the question.
